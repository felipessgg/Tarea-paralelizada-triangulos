...
#include <stdio.h>
#include <sstream>
#include <ctime>    //tiempo de ejecucion
...
int main()
{
    t0 = clock();
    creaArchivo();
    string triangulo1, triangulo2;
    std::cout << std::fixed;
    std::cout << std::setprecision(6);
    cout<<"RECUERDA!!"<<endl;
    cout<<"BORRAR PRIMER DATO '9665 3' DEL INICIO DEL ARCHIVO ESPIRAL.MESH\n"<<endl;
    cout<<"Ingrese numero de procesadores a utilizar: ";
    cin>>nroProcesadores;
    ...
    omp_set_num_threads(nroProcesadores);
    #pragma omp parallel
    {
        ...
        for(int i=0;i<9665;i++){
            if (lstCandidatos[i][1]==1){
                string mayor, linea;
                #pragma omp critical
                mayor = coordenadasPtoMedio(i); // ve que lado se partió

                int numNodoTmp = numNodo + omp_get_thread_num();
                //pasando numero numNodo a string
                std::stringstream numToString;
                numToString << numNodoTmp;

                if (mayor == "d1"){
                    //se intercambian nodos 1 y 2
                    triangulo1 = nod[i][0]+" "+numToString.str()+" "+nod[i][2];
                    triangulo2 = numToString.str()+" "+nod[i][1]+" "+nod[i][2];
                }
                if (mayor == "d2"){
                    //se intercambian nodos 2 y 3
                    triangulo1 = nod[i][0]+" "+numToString.str()+" "+nod[i][2];
                    triangulo2 = nod[i][0]+" "+nod[i][1]+" "+numToString.str();
                }
                if (mayor == "d3"){
                    //se intercambian nodos 3 y 1
                    triangulo1 = numToString.str()+" "+nod[i][1]+" "+nod[i][2];
                    triangulo2 = nod[i][0]+" "+nod[i][1]+" "+numToString.str();
                }
                #pragma omp critical
                ...
                #pragma omp critical
                linea = numToString.str()+" "+ptoMedio0ToString.str()+" "+ptoMedio1ToString.str();
                #pragma omp critical
                agregarLinea("resultados.node",linea);

                //double time = (double (clock()-t0)/CLOCKS_PER_SEC);
                //printf("FILA %d REFINADA / hilo nro %d / NUM NODO: %d / Tiempo: %f\n", i+1, omp_get_thread_num(), numNodoTmp, time);
                printf("FILA %d REFINADA\n", i+1);
                ...
                //double time = (double (clock()-t0)/CLOCKS_PER_SEC);
                //printf("FILA %d NO SE REFINA / Tiempo: %f\n", i+1, time);
                printf("FILA %d NO SE REFINA \n", i+1);
